#!/usr/bin/env node
/**
 * Creates slinger spritesheet from extracted PixelLab animations.
 * Uses east-facing direction frames.
 * Output: public/assets/sprites/enemies/slinger.png
 */
...
const EXTRACTED_PATH = '/tmp/slinger_extracted';
const OUTPUT_PATH = path.join(__dirname, '..', 'public/assets/sprites/enemies/slinger.png');
...
async function main() {
  console.log('Creating slinger spritesheet...');

  const idleFrames = getFrames('breathing-idle', 'east');
  const walkFrames = getFrames('walking', 'east');
  const attackFrames = getFrames('throw-object', 'east');
  const deathFrames = getFrames('falling-back-death', 'west'); // east failed, use west
